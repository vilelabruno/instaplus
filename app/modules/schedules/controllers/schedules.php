...
class schedules extends MX_Controller {
	...
	public function ajax_add_schedules(){
		...
		if(!check_expiration()  && session("admin") != 1){
			if(post('video_url') == ""){
				ms(array(
					"st"    => "valid",
					"label" => "bg-red",
		...
		if(!empty($activity)){
			...
			if($activity->status == 3 || $activity->status == 1){
				...
				ms(array(
					...
					"btn"   => '<button type="button" class="btn btn-danger btn-lg waves-effect uc btnActivityAll" style="width: 28%;">'.l('Stop').'</button>'
				));
			}else{
				$this->db->update(INSTAGRAM_SCHEDULES, array("status" => 3), "account_id = '".$activity->account_id."' AND category != 'post' AND category != 'message'");
				...
				ms(array(
					...
					"label" => "bg-green",
					"txt"   => l('Successfully'),
					"status"=> '<span class="badge btn-danger">'.l('Stoped').'</span> '.l('Activity'),
					"btn"   => '<button type="button" class="btn bg-light-green btn-lg waves-effect uc btnActivityAll" style="width: 28%;">'.l('Start').'</button>'
				));
			}
			
			//start all schedules
			$this->db->update(INSTAGRAM_SCHEDULES, array("status" => 5), "account_id = '".$activity->account_id."' AND category != 'post' AND category != 'message'");
